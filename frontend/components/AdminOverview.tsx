...
import { fetchJobTrends, type JobTrend } from "@/lib/api/jobMarket";
...
export default function AdminOverview({ adminName }: { adminName: string }) {
    ...
    return (
        <div style={{ maxWidth: 1400, margin: "0 auto", display: "flex", flexDirection: "column", gap: 28 }}>
            ...
            {/* Industrial Market Demand Section */}
            <div style={CARD}>
                <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 20 }}>
                    <div>
                        <h3 style={{ fontFamily: FONT_H, fontWeight: 800, color: V.text, fontSize: "1.1rem", margin: 0, display: "flex", alignItems: "center", gap: 8, letterSpacing: "-0.02em" }}>
                            <Brain size={18} color={V.accent} /> Industrial Skill Demand
                        </h3>
                        <p style={{ fontSize: "0.8rem", color: V.dim, marginTop: 4 }}>Real-time market analytics and salary benchmarks</p>
                    </div>
                    <div style={{ padding: "6px 12px", background: V.accentSoft, borderRadius: "8px", fontSize: "0.7rem", color: V.accent, fontWeight: 800 }}>
                        Market Data Active
                    </div>
                </div>
                ...
                <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 32 }}>
                    <div>
                        <ResponsiveContainer width="100%" height={260}>
                            <BarChart data={trends.slice(0, 6)} layout="vertical" margin={{ left: 40, right: 20 }}>
                                <XAxis type="number" hide />
                                <YAxis dataKey="skillName" type="category" tick={{ fill: V.text, fontSize: 11 }} width={80} axisLine={false} tickLine={false} />
                                <Tooltip {...TT} />
                                <Bar dataKey="jobCount" name="Job Openings" fill={V.accent} radius={[0, 4, 4, 0]} barSize={16} />
                            </BarChart>
                        </ResponsiveContainer>
                    </div>
                    <div style={{ display: "grid", gridTemplateColumns: "1fr", gap: 10 }}>
                        {trends.slice(0, 4).map(t => (
                            <div key={t.id} style={{ padding: "12px 16px", background: V.hover, borderRadius: 12, border: `1px solid ${V.border}`, display: "flex", justifyContent: "space-between", alignItems: "center" }}>
                                <div>
                                    <div style={{ fontWeight: 800, color: V.text, fontSize: "0.85rem" }}>{t.skillName}</div>
                                    <div style={{ fontSize: "0.7rem", color: V.dim, marginTop: 2 }}>Avg. Salary: <strong style={{ color: "#10b981" }}>${t.averageSalary.toLocaleString()}</strong></div>
                                </div>
                                <div style={{ fontSize: "0.65rem", fontWeight: 800, color: V.accent }}>{t.jobCount} POSITIONS</div>
                            </div>
                        ))}
                    </div>
                </div>
